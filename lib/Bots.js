...
class Bots {
		...
		static * getBtc(params) {
				const res = yield axios.get(`https://api.coindesk.com/v1/bpi/currentprice/${ params }.json`)
						.then(response => {
								const value = response.data.bpi[params.toUpperCase()].rate;

								return {
										response_type: 'in_channel', // public to the channel
										text: `The price of 1 bitcoin in ${ params.toUpperCase() } is $${ value }`,
										attachments: [
												{
														image_url: `https://bitcoincharts.com/charts/chart.png?width=940&m=bitstamp${ params.toUpperCase() }&SubmitButton=Draw&r=60&i=&c=0&s=&e=&Prev=&Next=&t=S&b=&a1=&m1=10&a2=&m2=25&x=0&i1=&i2=&i3=&i4=&v=1&cv=0&ps=0&l=0&p=0&`
												}
										]
								};
						})
				...
				const res = yield axios(`https://api.darksky.net/forecast/91cc800220a3ea89892cc21885b5e6c3/${ location.latlng.latitude },${ location.latlng.longitude }?units=auto`)
						.then(response => {
								...
								const daily = results.daily;
							        const unit = (results.flags.units == 'us') ? 'F' : 'C';

								return {
										response_type: 'in_channel', // public to the channel
										text: `Currently, it is ${ currently.temperature }&deg;${ unit } (${ currently.summary}) in ${ location.city }, ${ location.state }`,
